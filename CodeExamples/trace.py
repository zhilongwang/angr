...
def debug_func(state):
	f.write(str(state.se.constraints)) #this can give you the input constraints
	f.write("\n")
...
def main():
	...
	'''
	#load binary	
	proj = angr.Project('GetPathConstraint/test', load_options={'auto_load_libs': False})
	main = proj.loader.main_bin.get_symbol("main")
	#set entry point 
	state = proj.factory.blank_state(addr=main.addr)
	state.inspect.b('constraints', when=simuvex.BP_AFTER, action=debug_func)
	...
	"""
	for constr in p.state.se.constraints:
		f.write(str(constr))
	"""
	f.close()
	return
